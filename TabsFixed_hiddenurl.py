    ...
    async def hidecmd(self, message):
        ...
        if args or reply:
            if reply:
                ...
                except: await message.client.send_message(message.to_id, f'<a href="{args}">\u2060</a>')
            await message.delete()
        else: return await message.edit("Нет аргументов или реплая на медиа.")
      
    async def mhidecmd(self, message):
        args = utils.get_args_raw(message)
        if message:
            file = io.BytesIO(await message.client.download_file(message.media))
            file.name = message.file.name if message.file.name else message.file.id + message.file.ext
            try: x0at = requests.post('https://x0.at', files={'file': file})
            except ConnectionError as e: return await message.edit(str(e))
            await message.client.send_message(message.to_id, f'{args if args else ""} <a href="{x0at.text}">\u2060</a>')
        else:
            return await message.edit("Нет медиа с подписью.")
        await message.delete()