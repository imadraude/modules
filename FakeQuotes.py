...
class drooxHuotesMod(loader.Module):
    ...
    strings = {
        ...
        "silent_processing_cfg_doc": (
            "Process quote " "silently(mostly" " w/o editing)"
        ),
        ...
        "quote_limit_reached": (
            "The maximum number " "of messages in " "multiquote - {}."
        ),
    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig(
            ...
            "MODULE_ENDPOINT",
            "https://quotes.mishase.dev/f/module.py",
    ...
    async def fqcmd(self, message):
        ...
        if avatar:
            ...
            files.append(
                (
                    "files",
                    (f'@av{str(user.id).lstrip("-")}', open(avatar, "rb"), "image/jpg"),
                )
            )
        else:
        ...
        try:
            ...
            return await utils.answer(
                message, self.strings("unreachable_error", message)
            )
        await clean_files()
